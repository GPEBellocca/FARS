...
MutiLevelFactor3BlocksFULL<-function(Yorig,r,block_ind,tol){
  ...
  F12 = blockfact0(y_r12,c(NR1,NR2),1,c(1,1))  
  ...
  lamR1 = beta_ols(cbind(F12,F13),y_r1) 
  uR1L2 = y_r1-cbind(F12,F13) %*% t(beta_ols(cbind(F12,F13),y_r1))
  ...
  while ((log(uu0)-log(uu1))> tol){
    ...
    lam12=beta_ols(F12,ur12)
    lam13=beta_ols(F13,ur13)
    lam23=beta_ols(F23,ur23)
    
    
    lamR1 = beta_ols(cbind(F12,F13),uB[,1:NR1]) 
    ur1 = uB[,1:NR1]-cbind(F12,F13) %*% t(beta_ols(cbind(F12,F13),uB[,1:NR1]))
    ...
    F12 = F12-G0 %*% t(beta_ols(G0,F12))
    F23 = F23-G0 %*% t(beta_ols(G0,F23))
    
    
    R1x = fhatreg[,1]-cbind(F12,F13) %*% t(beta_ols(cbind(F12,F13),fhatreg[,1]))
    fhatreg[,1] = R1x
    R2x = fhatreg[,2]-cbind(F12,F23) %*% t(beta_ols(cbind(F12,F23),fhatreg[,2]))
    fhatreg[,2] = R2x
  ...
  V = (lam[,3]%*% (t(F12) %*%  F12)%*% t(lam[,3]))/t
  evec = eigrs2(V)$evec
  evec=t(lam[,3]) %*% evec[,ncol(evec)]
  F12 = F12 %*% evec
  
  V = (lam[,4]%*% (t(F23) %*%  F23)%*% t(lam[,4]))/t
  evec = eigrs2(V)$evec
  evec=t(lam[,4]) %*% evec[,ncol(evec)]
  F23 = F23 %*% evec
  
  
  # for(i in 1:g){
  #   evec<-eigrs2(t(fhatreg[,((i-1)*1+1):(i*1)]) %*% fhatreg[,((i-1)*1+1):(i*1)])$evec
  #   fhatreg[,((i-1)*1+1):(i*1)] <-fhatreg[,((i-1)*1+1):(i*1)]%*evec
  # }
  ...
  G0  = G0 -cbind(F13,F12,F23,fhatreg[,1],fhatreg[,2],fhatreg[,3]) %*% t(beta_ols(cbind(F13,F12,F23,fhatreg[,1],fhatreg[,2],fhatreg[,3]),G0))
  F13 = F13-cbind(G0,F12,F23,fhatreg[,1],fhatreg[,2],fhatreg[,3]) %*% t(beta_ols(cbind(G0,F12,F23,fhatreg[,1],fhatreg[,2],fhatreg[,3]),F13))
  F12 = F12-cbind(G0,F13,F23,fhatreg[,1],fhatreg[,2],fhatreg[,3]) %*% t(beta_ols(cbind(G0,F13,F23,fhatreg[,1],fhatreg[,2],fhatreg[,3]),F12))
  F23 = F23-cbind(G0,F12,F13,fhatreg[,1],fhatreg[,2],fhatreg[,3]) %*% t(beta_ols(cbind(G0,F12,F13,fhatreg[,1],fhatreg[,2],fhatreg[,3]),F23))
  F1   = fhatreg[,1] - cbind(G0,F13,F12,F23,fhatreg[,2],fhatreg[,3]) %*% t(beta_ols(cbind(G0,F13,F12,F23,fhatreg[,2],fhatreg[,3]),fhatreg[,1]))
  F2   = fhatreg[,2] - cbind(G0,F13,F12,F23,F1,fhatreg[,3]) %*% t(beta_ols(cbind(G0,F13,F12,F23,F1,fhatreg[,3]),fhatreg[,2]))
  F3   = fhatreg[,3] - cbind(G0,F13,F12,F23,F1,F2) %*% t(beta_ols(cbind(G0,F13,F12,F23,F1,F2),fhatreg[,3]))
  ...
  lam = cbind(lamG,
              ...
              c(lamr12[1:(NR1+NR2)],rep(0,NR3)),
              c(rep(0,NR1),lamr23[1:(NR2+NR3)]),
              c(lamr1,rep(0,NR2+NR3)),
              c(rep(0,NR1),lamr2,rep(0,NR3)),
              c(rep(0,NR1+NR2),lamr3))
  ...
  R[["Factors"]] <- cbind(G0,F13,F12,F23,F1,F2,F3)
  R[["Loadings"]] <- lam
  
  colnames(R[["Factors"]])<-c("G0","F13","F12","F23","F1","F2","F3")
  
  colnames(R[["Loadings"]])<-c("G0","F13","F12","F23","F1","F2","F3")
  
  return(R)
}


# round(cor(R$Factors),5)
# 
# plot(c(R$Factors[,1]),t="l")
# plot(c(R$Factors[,2]),t="l")
# plot(c(R$Factors[,3]),t="l")
# plot(c(R$Factors[,4]),t="l")
# plot(c(R$Factors[,5]),t="l")
# plot(c(R$Factors[,6]),t="l")
# plot(c(R$Factors[,7]),t="l")