...
get_Factors <- function(Factor_list, combination) {
...
MultipleBlocks3<-function(Yorig,r,block_ind,tol,max_iter){
  ...
  for (i in 1:(num_blocks-1)) {
    ...
    for (j in 1:nrow(combinations_matrix)) {
      ...
      if(r[factor_index]>0){
        ...
        if (i < num_blocks -1){
          # Middle levels
          Factors <- blockfact0(Residuals, num_vars[combination], 1, rep(1, length(combination)))
        }else{
          # Last level - Apply simple PCA
          number_of_factor <- 1 # number of factor to be extracted with PCA
